...
class UpdateCustomerLoan implements KeyListener,ActionListener
{
JFrame f;
...
im=new ImageIcon("WhatsApp Image customer loan 2.jpeg");
...
try
{
st=con.prepareStatement("Update customerloan set loan_type=?,customer_name=?,rate_of_intrest=?,loan_amount=?,time_period=?,description=?,date=? where account_number=?");
st.setString(1,lontyp);
st.setString(2,cusnm);
st.setString(3,roi);
st.setString(4,loamt);
st.setString(5,tp);
st.setString(6,des);
st.setString(7,dt);
st.setString(8,accn);
