...
class UpdateCustomerPolicy implements KeyListener,ActionListener
{
JFrame f;
...
im=new ImageIcon("WhatsApp Image Customern policy 2.jpeg");
...
try
{
st=con.prepareStatement("Update customerpolicy set customer_name=?,policy_name=?,benefits=?,time_period=?,date=?,amount=? where account_no=?");
st.setString(1,cusnm);
st.setString(2,pnm);
st.setString(3,bf);
st.setString(4,tp);
st.setString(5,dt);
...
class UpdateCustomerPolicyDemo
{
public static void main(String[] a)
{
UpdateCustomerPolicy l=new UpdateCustomerPolicy();
}
}
