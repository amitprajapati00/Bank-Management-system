...
class LoanDeposit implements KeyListener,ActionListener
{
JFrame f;
...
ResultSet rs;
String accno,cusnm,amt,dt,mod;


LoanDeposit()
{
f=new JFrame("Loan Deposit");
f.setSize(700,500);
...
im=new ImageIcon("WhatsApp Image Loan Deposite.jpeg");
...
l4=new JLabel("Total Amount Deposite");
...
st=con.prepareStatement("select account_number from customerloan");
rs=st.executeQuery();
while(rs.next())
{
...
dt=t3.getText();
amt=t4.getText();
mod=t5.getSelectedItem().toString();

try
{
st=con.prepareStatement("insert into deposite values(?,?,?,?,?)");
...
st.setString(5,mod);
st.executeUpdate();
JOptionPane.showMessageDialog(f,"Deposite Successfully");
...
if(rs.next())
   intbal=Integer.parseInt(rs.getString("loan_amount"));

tot=intbal-amt1;
st=con.prepareStatement("Update customerloan set loan_amount=? where account_number=?");
...
class LoanDepositDemo
{
public static void main(String[] a)
{
LoanDeposit l=new LoanDeposit ();
}
}
